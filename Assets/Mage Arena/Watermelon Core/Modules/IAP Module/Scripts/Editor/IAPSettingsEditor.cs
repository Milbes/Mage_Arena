    ...
    public class IAPSettingsEditor : WatermelonEditor
    {
        ...
        private class IAPTypesWindow : WatermelonWindow
        {
            ...
            protected override void OnEnable()
            {
                ...
                string[] manager = AssetDatabase.FindAssets("IAPManager t:MonoScript");
                if (manager.Length > 0)
                {
                    folderPath = Path.GetDirectoryName(AssetDatabase.GetAssetPath((MonoScript)AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(manager[0]), typeof(MonoScript))));
                }
