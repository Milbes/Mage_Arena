...
{
    public class TurtleEnemy : EnemyBehavior
    {
        [Space]
        ...
        private bool IsPlayerInRange { get => (Position - PlayerController.Position).magnitude <= turtleData.ShootingDistance; }
        ...
        {
            turtleData = enemyData as TurtleData;
        ...
        {
            if (Time.time - idleStartTime < turtleData.IdlePhaseDuration)
            {
        ...
        private void MovementStateUpdate()
        {
            ...
            if(travercedDistance >= turtleData.RunningMaxDistance)
            {
                if (IsPlayerInRange)
                {
                    InitAttackState();
            ...
            if (pathRecalculationTime >= turtleData.PathRecalculationDuratuion)
            {
                RecalculatePath();
            }
            ...
            if (distance.magnitude > turtleData.Speed * Time.deltaTime)
            {
                Vector3 framePath = distance.normalized * turtleData.Speed * Time.deltaTime;
        ...
        private void InitAttackState()
        {
            ...
            delayCase1 = Tween.DelayedCall(turtleData.IdleBeforeAttack, () => {

                if(!IsDying) attackCollider.gameObject.SetActive(true);
